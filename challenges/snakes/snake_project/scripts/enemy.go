...
func CreateEnemySnake(g *Game) *EnemySnake {
	...
	sHeadU, _, _ := ebitenutil.NewImageFromFile("img_res/eHeadUp.png", ebiten.FilterDefault)
	sHeadD, _, _ := ebitenutil.NewImageFromFile("img_res/eHeadDown.png", ebiten.FilterDefault)
	sHeadL, _, _ := ebitenutil.NewImageFromFile("img_res/eHeadLeft.png", ebiten.FilterDefault)
	sHeadR, _, _ := ebitenutil.NewImageFromFile("img_res/eHeadRight.png", ebiten.FilterDefault)
	bodyE, _, _ := ebitenutil.NewImageFromFile("img_res/eBody.png", ebiten.FilterDefault)
	e.sHeadU = *sHeadU
	e.sHeadD = *sHeadD
	e.sHeadL = *sHeadL
	e.sHeadR = *sHeadR
...
func (enemy *EnemySnake) Direction(dotTime int) error {
	if dotTime == 1 {
		...
		if changingDirection == 0 {
			switch action {
			case 0:
				if posX < 40 && enemy.lastDir != "left" {
					enemy.lastDir = "right"
				} else {
					enemy.lastDir = "left"
				}
			case 1:
				if posY < 80 && enemy.lastDir != "up" {
					enemy.lastDir = "down"
				} else {
					enemy.lastDir = "up"
				}
			case 2:
				if posY > 640 && enemy.lastDir != "down" {
					enemy.lastDir = "up"
				} else {
					enemy.lastDir = "down"
				}
			case 3:
				if posX > 540 && enemy.lastDir != "right" {
					enemy.lastDir = "left"
				} else {
					enemy.lastDir = "right"
				}
